...
### Constructor signature is mandatory in class declared in header file!

In C++, a class constructor **must be declared in the header file** if it is user-defined and not implicitly generated by the compiler. Here's the reasoning:

1. **Declaration vs. Definition**:  
   - The **declaration** of a constructor (like any member function) must appear in the class definition within the header file. This informs the compiler about the constructor's existence and signature (parameters, access level, etc.).  
   - The **definition** (implementation) can be placed either inline in the header or in a separate `.cpp` file. If defined in a `.cpp` file, the header must still declare it.

2. **Compiler Requirements**:  
   When code creates an instance of the class (e.g., `MyClass obj;`), the compiler must see the constructor's declaration in the header to:  
   - Verify the constructor exists and is accessible.  
   - Enforce type safety by checking parameters (if any).  
   - Avoid compilation errors like "no matching constructor" or "constructor not declared."

3. **Implicit Constructors**:  
   - If no constructors are declared, the compiler implicitly generates a default constructor. In this case, no explicit declaration is needed in the header.  
   - If you declare **any** user-defined constructor (e.g., `MyClass(int x);`), the compiler no longer generates the implicit default constructor. You must explicitly declare a default constructor (e.g., `MyClass() = default;`) in the header if needed.

4. **Examples**:  
   - **Declaration in Header, Definition in `.cpp`**:  
     ```cpp
     // MyClass.h
     class MyClass {
     public:
         MyClass(); // Declaration in header
     };

     // MyClass.cpp
     MyClass::MyClass() { ... } // Definition in .cpp
     ```
   ...
   - **Inline Definition in Header**:  
     ...
     class MyClass {
     public:
         MyClass() { ... } // Declaration + inline definition
     };
     ```

5. **Key Exceptions**:  
   - **`= default` Syntax**: You can explicitly request a compiler-generated constructor in the header without defining it:  
     ```cpp
     class MyClass {
     public:
         MyClass() = default; // Declaration + compiler-generated definition
     };
     ```
   - **Implicit Default Constructor**: No declaration needed if no user-defined constructors exist.

**Summary**:  
Always declare user-defined constructors in the header to ensure proper visibility for code using the class. Definitions can be inline (in the header) or in a `.cpp` file. Implicit or `= default` constructors may omit explicit declarations if they meet compiler-generated criteria.

## Constructor implementation guidance in Implementation (`.cpp`) files

To implement a constructor in a **.cpp file** for a class declared in a header, follow these steps:

---

### 1. **Declare the Constructor in the Header File**
   - In your `.h`/`.hpp` file, declare the constructor inside the class definition.  
   - Example:  
     ```cpp
     // MyClass.h
     #pragma once
     ...
     class MyClass {
     public:
         MyClass(int x); // Declaration
         MyClass();       // Default constructor (optional)
     private:
         int value;
     };
     ```
...
### 2. **Define the Constructor in the .cpp File**
   - In the `.cpp` file, use the **scope resolution operator `::`** to define the constructor.  
   - Include parameters and initialization lists as needed.  
   - Example:  
     ```cpp
     // MyClass.cpp
     #include "MyClass.h"
     ...
     MyClass::MyClass(int x) : value(x) {  // Initializer list
         // Additional logic (if needed)
     }

     // Default constructor
     MyClass::MyClass() : value(0) { }     // Initialize value to 0
     ```
...
### Key Rules:
1. **Initializer Lists**:  
   Place member initializations in the initializer list (after the `:`) for efficiency and correctness.  
   ```cpp
   MyClass::MyClass(int x) : value(x) { ... }
   ```

2. **Parameter Matching**:  
   The parameters in the `.cpp` definition **must match** the declaration in the header.  

3. **Default Constructors**:  
   If declaring a default constructor, define it explicitly in the `.cpp` file:  
   ```cpp
   MyClass::MyClass() { ... }
   ```
...
- **Implementation (`MyClass.cpp`)**  
  ...
  MyClass::MyClass(int x) : value(x) {
      // Additional code (e.g., logging, validation)
  }
...
### Why This Works:
- The header declares the class's interface, including constructor signatures.  
- The `.cpp` file provides the implementation details.  
- The linker connects the constructor declaration (from the header) to its definition (in the `.cpp`).

---

### Common Pitfalls:
- **Missing Scope Resolution (`::`)**:  
  Incorrect: `MyClass(int x) { ... }`  
  Correct: `MyClass::MyClass(int x) { ... }`

- **Mismatched Parameters**:  
  Ensure the `.h` and `.cpp` parameter lists match exactly.

- **Forgetting to Include the Header**:  
  The `.cpp` file must `#include "MyClass.h"`.

---

By separating declaration (header) and definition (implementation), you keep your code organized and adhere to best practices in C++.
